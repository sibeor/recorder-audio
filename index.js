...
async function sendResultToServer (success, fileName) {
  try {
    ...
    if (response.status === 200) {
      ...
      console.error('A intervenit o eroare la trimiterea mesajului către server.')
    }
  } catch (error) {
    console.error('A intervenit o eroare la trimiterea mesajului către server:', error.message)
  }
}
...
function awaitRecorder (recorder) {
  return new Promise((resolve, reject) => {
    console.log('recorder')
    console.log(recorder)
    setInterval((recorder) => {
      if (recorder?.isRecording === false || !recorder?.isRecording) {
        resolve(true)
      }
    }, 1000)
  })
